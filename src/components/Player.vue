<template>
    <div class="player" ref="playerFather">
            ...
            <v-container class="pa-2">
                <v-row no-gutters justify="space-between">
                    ...
                    <v-col cols="6" class="pl-0 d-flex flex-column justify-space-around">
                        ...
                        <p class="mb-0 d-inline-block text-truncate font-weight-light" style="max-width: 100%; font-size: 0.8rem">{{playArticles}}</p>
                    </v-col>
                    <v-col cols="3" class="d-flex align-center mr-2">
                        <div class="d-flex justify-space-between" style="width: 100%">
                            <v-btn small icon color="green accent-4" @click="previous"><v-icon>mdi mdi-skip-previous</v-icon></v-btn>
                            <v-btn small icon color="green accent-4" @click="play" v-if="playing && !player.ended"><v-icon>mdi mdi-pause-circle-outline</v-icon></v-btn>
                            <v-btn small icon color="green accent-4" @click="play" v-else><v-icon>mdi mdi-play-circle-outline</v-icon></v-btn>
                            <v-btn small icon color="green accent-4" @click="next"><v-icon>mdi mdi-skip-next</v-icon></v-btn>
                        </div>
                    </v-col>
                </v-row>
            </v-container>
