...
export default {
    ...
    methods: {
        async _getRank() {
            const temp = window.sessionStorage.getItem("topList")
            if(temp) {
                this.rankList = JSON.parse(temp)
                return
            }
            // const rankListUrl = "/api/getTopLists"
            ...
            try {
                ...
                if (response.status === 200 && response.data.code === 0) {
                    ...
                    if (data.code === 0) {
                        ...
                        window.sessionStorage.setItem("topList", JSON.stringify(data.data.group))
                    }
                }else {
